...
export class UserService {
  ...
  addItem(data) {
    this.userlists.value.unshift(data);
    this.userlists.next(this.userlists.value);
  }

  updateItem(data) {
    let index = this.userlists.value.map(item => item.id).indexOf(data.id);
  ...
  removeItem(data) {
    ...
    this.userlists.next(this.userlists.value.filter(item => item.id !== data.id));
  }
  create(user: any) {
    const httpOptions = {
  ...
  showUserById(id: string) {
    ...
    return this.http.get(this.baseUrl + '/user/showUserById/' + id, httpOptions)
      .pipe(
        retry(1),
        catchError(this.errorHandler)
      )
